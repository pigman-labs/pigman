---
source_file: "tests/unit/test_torch_backend.py"
type: "code"
community: "activations_l2_normalize, activations_tanh, activations_tanh_grad"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/activations_l2_normalize,_activations_tanh,_activations_tanh_grad
---

# test_torch_backend.py

## Connections
- [[test_torch_dynamics_forward_shapes()]] - `contains` [EXTRACTED]
- [[test_torch_jepa_forward_shapes()]] - `contains` [EXTRACTED]
- [[test_torch_training_writes_checkpoints()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/activations_l2_normalize,_activations_tanh,_activations_tanh_grad